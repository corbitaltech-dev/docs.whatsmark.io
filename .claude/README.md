# WaMetrix.io docs — the `.claude/` layer

This folder holds the agents and skills that write, draft, and monitor the
customer-facing documentation at `docs.WaMetrix.io`. It's organised as two
...
├── skills/
│   ├── WaMetrix-docs/SKILL.md            — the rulebook: voice, brand, banned words, structure, Mintlify mechanics
...
| Layer | Files | Job |
| --- | --- | --- |
| **Rulebook** | `WaMetrix-docs` | The authoritative rules: voice, banned words, brand, mechanics, Meta citations, shared-repo safety, checklist |
...
Nothing is duplicated — every new file points back to `WaMetrix-docs` as the single source of truth, the same way `docs-writer` does.
...
**Auto-fix only the unambiguous, mechanical things; escalate everything with a judgement, a fact, or a voice rewrite.** The split is defined once, in `docs-manager` ("Decision model"), and every skill defers to it. And the shared-repo rule from `WaMetrix-docs` §7 always holds: touch only the files a task owns, keep `docs.json` diffs tiny, never overwrite another writer's recent work.
...
| "tenant", bare "WaMetrix", "Free plan" | `docs-consistency-lint` §1–3 |
...
| Empty `AGENTS.md` boilerplate | fixed 2026-07-27 — `AGENTS.md` now points at `WaMetrix-docs` |
