---
name: docs-drafter
description: Turns short feature notes plus screenshots into a finished, on-brand docs.WaMetrix.io page. Reads the screenshots for real UI labels, verifies every fact against the product before drafting, and writes a complete MDX page following the house structure and voice. Use when the user hands over a filled INTAKE-TEMPLATE.md, loose bullet notes with screenshots, or says "draft a page from these notes/screenshots".
...
- **`WaMetrix-docs`** — the underlying voice, brand, and structure rulebook the drafted page must follow.
...
- "WaMetrix.io", never bare "WaMetrix", except in `keywords:`.
