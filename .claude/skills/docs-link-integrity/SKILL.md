...
Run this after *any* edit to `docs.json`, including ones made by another skill — it's the single file most likely to be mid-edit by another writer (`wamatrix-docs` §7), so a syntax error here breaks the whole site's navigation, not just one page.
