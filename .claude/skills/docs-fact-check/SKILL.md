...
**Product source:** `/media/corbital/web_data/laravel.local/wamatrix.io` — Laravel + Vue/Inertia (`resources/js/pages`, no `app/Livewire`).

⚠️ **Do not use `/media/corbital/web_data/laravel.local/wamatrix-saas`.** That is the older Livewire v1 product and does **not** match what the docs describe. Confirm you have the right repo before grepping: the current one has `resources/js/pages` and an empty `app/Livewire`.
...
`/media/corbital/web_data/laravel.local/wamatrix.io/developer-docs/`. That folder is the
...
- **Every limit or price** — and remember these belong only in `getting-started/free-vs-paid.mdx` (see the `wamatrix-docs` skill).
...
```bash
cd /media/corbital/web_data/laravel.local/wamatrix.io
