...
**Read `WaMetrix-docs` first** — it sets the voice, banned words, brand and plan naming, Mintlify mechanics, Meta-citation rules, and the page structure you build to. This skill is the intake-to-draft method on top of it. Verify facts with `docs-fact-check`; check the finished page with `docs-reviewer`.
...
Map the notes to the proven pattern (`WaMetrix-docs` §4). **Use only the sections that fit** — a small feature page is often just hook → steps → what's next. Don't force all eight.
...
Momentum rule: "Keep going" and "What's next" point to pages the reader **hasn't** seen yet, never backwards (`WaMetrix-docs` §1a).
...
keywords: ["WaMetrix", "WaMetrix.io", "…"]   # bare WaMetrix allowed here only
---
```
...
keywords: ["WaMetrix", "WaMetrix.io", "canned replies", "quick replies", "shortcuts", "live chat"]
---
