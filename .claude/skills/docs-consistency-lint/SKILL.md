...
## §2 — Bare "WhatsMark" instead of "WaMatrix.io"

```bash
grep -rnE --include="*.mdx" "WhatsMark[^.]" . | grep -v "keywords"
```

Every hit outside a `keywords:` array should read **WaMatrix.io**. Watch for the regex matching a sentence end (`WhatsMark.` with a period, which is correct) — read the actual match, don't blind-replace.
